...
export const getLatestReleasesContent = async (): Promise<Content[]> => {
  const query = `  SELECT c.id, c.name, c.release_date::DATE, c.watch_time, c.description, c.poster, c.trailer_link, ct.type,COALESCE(AVG(r.stars)::NUMERIC(10,1), 0)::FLOAT AS average_rating
                   FROM content c JOIN content_type ct ON c.type_id = ct.id 
                   LEFT JOIN reviews r ON c.id = r.content_id 
                   GROUP BY c.id, ct.type
...
export const getTopRatedContent = async (typeId: number | null, year: number | null): Promise<Content[]> => {
  const query = `SELECT c.id, c.name, c.release_date::DATE, c.watch_time, c.description, c.poster, c.trailer_link, ct.type,COALESCE(AVG(r.stars)::NUMERIC(10,1), 0)::FLOAT AS average_rating
                   FROM content c JOIN content_type ct ON c.type_id = ct.id 
                   LEFT JOIN reviews r ON c.id = r.content_id
                   WHERE (c.type_id= $1 OR $1 IS NULL) AND (EXTRACT(YEAR FROM c.release_date)= $2 or $2 IS NULL)
                   GROUP BY c.id, ct.type
                   ORDER BY average_rating DESC
                   LIMIT 10`;

  const data = [typeId, year];
  const res = await pool.query(query, data);
  return res.rows;
};
